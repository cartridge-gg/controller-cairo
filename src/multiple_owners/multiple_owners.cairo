...
mod multiple_owners_component {
    ...
    > of IMultipleOwners<ComponentState<TContractState>> {
        ...
        fn assert_valid_owner_signature(self: @ComponentState<TContractState>, signer_signature: SignerSignature) {
            ...
            let message_hash = poseidon_hash_span(
                array![selector!("add_owner"), get_tx_info().unbox().chain_id, get_contract_address().into()].span(),
            );
