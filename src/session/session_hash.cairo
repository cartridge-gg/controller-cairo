...
use controller::session::interface::{Policy, Session, TypedData};
...
const DATA_TYPE_HASH_REV_1: felt252 = selector!(
    "\"Allowed Type\"(\"Scope Hash\":\"felt\",\"Typed Data Hash\":\"felt\")",
);
...
const ALLOWED_DATA_TYPE_HASH_REV_1: felt252 = selector!("\"Allowed Type\"(\"Scope Hash\":\"felt\")");
...
impl StructHashTypedData of IStructHashRev1<TypedData> {
    fn get_struct_hash_rev_1(self: @TypedData) -> felt252 {
        let self = *self;
        poseidon_hash_span(array![DATA_TYPE_HASH_REV_1, self.scope_hash, self.typed_data_hash].span())
    }
}
