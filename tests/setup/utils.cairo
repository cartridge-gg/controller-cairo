use controller::signer::signer_signature::{Signer, SignerSignature, SignerTrait, StarknetSignature, StarknetSigner};
...
fn to_starknet_signer_signatures(arr: Array<felt252>) -> Array<felt252> {
    ...
    while let Option::Some(item) = arr.pop_front() {
        let pubkey = (*item).try_into().expect('ctrl/zero-pubkey');
...
fn to_starknet_signatures(arr: Array<KeyAndSig>) -> Array<felt252> {
    ...
    let mut arr = arr.span();
    while let Option::Some(item) = arr.pop_front() {
        let pubkey = (*item.pubkey).try_into().expect('ctrl/zero-pubkey');
